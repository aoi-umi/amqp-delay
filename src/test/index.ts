...
import { MQ, defaultDelayTime } from "../lib";
    ...
    await mq.ch.addSetup(async (ch: ConfirmChannel) => {
        ...
        return Promise.all([
            ...
            mq.consumeRetry(ch, delayConfig.deadLetterQueue, async (content) => {
                console.log(content);
                let now = new Date();
                throw new Error(`at [${now}], delay:[${(now.getTime() - new Date(content.startAt).getTime()) / 1000}] test`)
            }),
        ] as any);
    });

    mq.sendToQueueDelayByConfig(delayConfig, { startAt: new Date(), text: 'dealy and retry' });
})()